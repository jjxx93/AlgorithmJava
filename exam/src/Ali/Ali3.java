...
public class Ali3 {
    ...
    public static List<String> calculateUnilateral(List<UnilateralLine> lineList) {
        ...
        for (int i = 0; i < lineList.size(); i++) {
            String firstPro = lineList.get(i).sPro;
            String priEnd = lineList.get(i).ePro;
            String tType = lineList.get(i).tType;
            StringBuilder line = new StringBuilder(lineList.get(i).id).append('+');
            Boolean isContinue = true;

            while (isContinue) {
                isContinue = false;
                for (int j = 0; j < lineList.size(); j++) {
                    UnilateralLine unilateralLine = lineList.get(j);

                    if (unilateralLine.sPro.equals(priEnd)) {
                        if (unilateralLine.tType.equals(tType)) {
                            if (unilateralLine.ePro.equals(firstPro)) {
                                line.append(unilateralLine.id);
                                lineList.remove(unilateralLine);
                                break;
                            } else {
                                line.append(unilateralLine.id).append('+');
                                priEnd = unilateralLine.ePro;
                                isContinue = true;
                                lineList.remove(unilateralLine);
                                break;
                            }
                            ...
                            for (int k = j + 1; k < lineList.size(); k++) {
                                unilateralLine = lineList.get(k);
                                if (unilateralLine.sPro.equals(priEnd) && !unilateralLine.tType.equals(tType)) {
                                    if (unilateralLine.ePro.equals(firstPro)) {
                                        line.append(unilateralLine.id);
                                        lineList.remove(unilateralLine);
                                        break;
